...
def spawn_enemy(game):
    """
    Spawn an enemy in the current area based on area type.
    ...
    if current_area and current_area.area_type != "town" and len(current_area.enemies) < 3:
        ...
        enemy = Enemy(game.player.level if game.player else 1)
...
def check_boss_battle_trigger(game):
    """
    Check if a boss battle should be triggered after level up.
    
    Args:
        game: The main Game instance
        
    Returns:
        bool: True if boss battle was triggered, False otherwise
    """
    if (
        game.player.just_leveled_up and
        game.player.level > 1 and
        not game.player.boss_cooldown and
        game.player.level > game.player.last_boss_level
    ):
        # At level 10, spawn Malakor, else progressive boss
        if game.player.level == 10:
            game.battle_screen = game.battle_screen.__class__(game.player, BossDragon())
        else:
            game.battle_screen = game.battle_screen.__class__(game.player, DragonBoss(game.player.level))
        game.battle_screen.start_transition()
        game.state = "battle"
        game.player.boss_cooldown = True  # Prevent immediate retrigger
