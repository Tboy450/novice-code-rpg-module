...
class DragonBoss(Enemy):
    ...
    def __init__(self, boss_level):
        super().__init__(player_level=boss_level)
        self.size = 120
        self.x = 700
        self.y = 180
        self.enemy_type = "dragon_boss"
    ...
    def draw(self, surface):
        """Draw the dragon boss with detailed graphics and animations"""
        ...
        # Wings (bat-like, animated)
        wing_y = y + 60
        wing_offset = math.sin(pygame.time.get_ticks() * 0.01) * 5
        ...
        # Fire breath animation (large cone from mouth to player)
        if self.fire_breathing:
            mouth_x = head_x - 10
            mouth_y = head_y + 40
            player_x = 200 + 25
...
class BossDragon(Enemy):
    ...
    def draw(self, surface):
        """Draw the final boss dragon with enhanced graphics and effects"""
        # Calculate animation offsets
        offset_x = 0
        offset_y = self.animation_offset
        ...
        # Draw aura effect for final boss
        ...
        eye_glow = math.sin(pygame.time.get_ticks() * 0.02) * 0.3 + 0.7
        glow_color = (int(255 * eye_glow), 0, 0)  # Red evil glow
        pygame.draw.circle(surface, glow_color, (head_x + 15, head_y + 15), 7)
        pygame.draw.circle(surface, (0, 0, 0), (head_x + 13, head_y + 15), 3)
        
        # Devastating fire breath animation
        if self.fire_breathing:
            mouth_x = head_x - 10
            mouth_y = head_y + 40
            player_x = 200 + 25
